...
- `currency` is an ISO-4217 3-letter code validated against the **`currencies`
  ...
  hardcoded enum sets across onboarding/settings (the old VND/USD/EUR vs
  VND/USD/THB inconsistency). Snapshot totals are in `household.currency`.
...
- **backend**: household fields on the `households` table; `currency` FK to the `currencies` reference table; sharing enforced app-layer via visibility levels.
- **mobile-app**: to be ported.

## Enums

`SharingLevel = overview | grouped | detailed`, `currency` = ISO-4217 code (FK to `currencies`), `updateFrequency = weekly | biweekly | monthly`.
