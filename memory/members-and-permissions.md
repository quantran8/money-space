...
Permissions are enforced in the **backend app layer** (NestJS guards + query
filters), **not** Postgres RLS, so the project stays DB-portable. Two guards run
globally:
- **`SupabaseAuthGuard`** — authenticates every route (populates the request
  user). Skipped for `@Public()` routes (auth signup/login/google/refresh +
  worker accrue-interest endpoints).
- **`HouseholdAccessGuard`** — on `/api/households/:householdId/*`, verifies the
  user is a live member, resolves their effective membership (`role`, effective
  `permission`, `isOwner`), and enforces the route's required capability.

**2-axis model**: capability (role/permission) + visibility (record sensitivity).
- **Capability**: `effectivePermission(role, override) = override ?? defaultPermissionForRole`.
  `@RequireCapability('edit')` guards all create/update/delete routes (assets,
  debts, goals, payments, money-events, snapshots); `@RequireCapability('admin')`
  guards member management. Read routes need no decorator — any member may read;
  visibility is gated per-record. `canEdit` = permission ≥ `edit_content`;
  `canAdmin` = `admin`.
- **Visibility** (`canViewVisibility`): `visible = viewer.tier ≥ record.tier AND
  (record ≠ private OR viewer is creator/admin)`. Tiers: summary_only < grouped <
  detail; `private` = creator/admin only.
...
- **`permissionLevel` is a nullable OVERRIDE** (both on `household_members` and
  ...
  a permission differing from their role default. The backend derives on read when
  the DB value is NULL. This is the "2-axis" model: role/capability + record
  visibility tier.
...
- Member status: `active | invited` — a **persisted column** on `household_members`
  (not hardcoded); set on create (default `invited`). Removing a member
  **soft-deletes** (`deletedAt`) rather than hard-deletes, so audit logs and owned
  assets/debts keep their FK references; all member queries filter `deletedAt IS NULL`.

## Visibility / sharing

`VisibilityLevel = summary_only | grouped | detail | private` is a per-entity sharing control that drives the privacy model (enforced app-layer, not RLS). See [[settings-and-sharing]].
