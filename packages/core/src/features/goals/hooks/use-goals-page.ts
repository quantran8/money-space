...
export function useGoalsPage() {
  ...
  async function onSubmit(values: GoalForm) {
    try {
      ...
      if (editingId) {
        ...
        notify.success(t('goals.toast.updated'))
      } else {
        ...
        notify.success(t('goals.toast.created'))
      }
  ...
  async function handleDeleteGoal(goalId: string) {
    try {
      ...
      notify.success(t('goals.toast.deleted'))
      if (editingId === goalId) handleFormOpenChange(false)
    } catch (error) {
      notify.error(getErrorMessage(error, t('goals.toast.deleteFailed')))
      throw error
    }
  }
  ...
  async function addAllocation(goalId: string, payload: GoalAllocationPayload) {
    try {
      ...
      notify.success(t('goals.toast.allocationSaved'))
      return true
    } catch (error) {
      notify.error(getErrorMessage(error, t('goals.toast.allocationFailed')))
      return false
    }
  }
  ...
  async function removeAllocation(goalId: string, allocationId: string) {
    try {
      ...
      notify.success(t('goals.toast.allocationRemoved'))
      return true
    } catch (error) {
      notify.error(getErrorMessage(error, t('goals.toast.allocationFailed')))
      return false
