...
import { useLocation, useNavigate, useSearchParam } from '#/shared/navigation'
...
export function useJoinInvite() {
  ...
  const token = useSearchParam(JOIN_PARAM_TOKEN)?.trim() ?? ''
  const householdId = useSearchParam(JOIN_PARAM_HOUSEHOLD)?.trim() ?? ''
  ...
  const accept = useMutation({
    ...
    onError: (cause) => {
      notify.error(getErrorMessage(cause, t('invites.join.failed')))
