...
function Switch({
  ...
  return (
    <SwitchPrimitive.Root
      data-slot="switch"
      className={cn(
        'peer inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border border-ink3/40 bg-wash transition-colors focus-visible:outline-2 focus-visible:outline-action focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:border-action data-[state=checked]:bg-action',
        className,
      )}
      {...props}
    >
      <SwitchPrimitive.Thumb
        ...
        className="pointer-events-none block size-5 rounded-full bg-ink3 ring-0 transition-[transform,background-color] data-[state=checked]:translate-x-5 data-[state=checked]:bg-card data-[state=unchecked]:translate-x-0.5"
      />
    </SwitchPrimitive.Root>
  )
}
